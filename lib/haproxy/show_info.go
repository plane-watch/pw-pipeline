...
type (
	Info struct {
		...
		// NbProc: Number of started worker processes (historical, always 1)
		NbProc uint32 `haproxy:"Nbproc"`

		// ProcessNum: Relative worker process number (1)
		ProcessNum uint32 `haproxy:"Process_num"`

		// Pid: This worker process identifier for the system
		Pid uint32 `haproxy:"Pid"`

		// Uptime: How long ago this worker process was started (days+hours+minutes+seconds)
		Uptime time.Duration `haproxy:"Uptime"`

		// UptimeSec: How long ago this worker process was started (seconds)
		...
		// PoolFailed: Number of failed pool allocations since this worker was started
		...
		// CurrConns: Current number of connections on this worker process
		CurrConns uint32 `haproxy:"CurrConns"`

		// CumConns: Total number of connections on this worker process since started
		CumConns uint32 `haproxy:"CumConns"`

		// CumReq: Total number of requests on this worker process since started
		...
		// CurrSslConns: Current number of SSL endpoints on this worker process (front+back)
		CurrSslConns uint32 `haproxy:"CurrSslConns"`

		// CumSslConns: Total number of SSL endpoints on this worker process since started (front+back)
		...
		// PipesUsed: Current number of pipes in use in this worker process
		PipesUsed uint32 `haproxy:"PipesUsed"`

		// PipesFree: Current number of allocated and available pipes in this worker process
		PipesFree uint32 `haproxy:"PipesFree"`

		// ConnRate: Number of front connections created on this worker process over the last second
		...
		// MaxConnRate: Highest ConnRate reached on this worker process since started (in connections per second)
		MaxConnRate uint32 `haproxy:"MaxConnRate"`

		// SessRate: Number of sessions created on this worker process over the last second
		...
		// MaxSessRate: Highest SessRate reached on this worker process since started (in sessions per second)
		MaxSessRate uint32 `haproxy:"MaxSessRate"`

		// SslRate: Number of SSL connections created on this worker process over the last second
		...
		// MaxSslRate: Highest SslRate reached on this worker process since started (in connections per second)
		MaxSslRate uint32 `haproxy:"MaxSslRate"`

		// SslFrontendKeyRate: Number of SSL keys created on frontends in this worker process over the last second
		SslFrontendKeyRate uint32 `haproxy:"SslFrontendKeyRate"`

		// SslFrontendMaxKeyRate: Highest SslFrontendKeyRate reached on this worker process since started (in SSL keys per second)
		...
		// SslBackendKeyRate: Number of SSL keys created on backends in this worker process over the last second
		SslBackendKeyRate uint32 `haproxy:"SslBackendKeyRate"`

		// SslBackendMaxKeyRate: Highest SslBackendKeyRate reached on this worker process since started (in SSL keys per second)
		SslBackendMaxKeyRate uint32 `haproxy:"SslBackendMaxKeyRate"`

		// SslCacheLookups: Total number of SSL session ID lookups in the SSL session cache on this worker since started
		SslCacheLookups uint32 `haproxy:"SslCacheLookups"`

		// SslCacheMisses: Total number of SSL session ID lookups that didn't find a session in the SSL session cache on this worker since started
		SslCacheMisses uint32 `haproxy:"SslCacheMisses"`

		// CompressBpsIn: Number of bytes submitted to the HTTP compressor in this worker process over the last second
		CompressBpsIn uint32 `haproxy:"CompressBpsIn"`

		// CompressBpsOut: Number of bytes emitted by the HTTP compressor in this worker process over the last second
		...
		// Tasks: Total number of tasks in the current worker process (active + sleeping)
		Tasks uint32 `haproxy:"Tasks"`

		// RunQueue: Total number of active tasks+tasklets in the current worker process
		RunQueue uint32 `haproxy:"Run_queue"`

		// IdlePercent: Percentage of last second spent waiting in the current worker thread
		IdlePercent uint32 `haproxy:"Idle_pct"`

		// Node: Node name (global.node)
		Node string `haproxy:"node"`

		// Stopping: 1 if the worker process is currently stopping, otherwise zero
		Stopping uint32 `haproxy:"Stopping"`

		// Jobs: Current number of active jobs on the current worker process (frontend connections, master connections, listeners)
		Jobs uint32 `haproxy:"Jobs"`

		// Unstoppable Jobs: Current number of unstoppable jobs on the current worker process (master connections)
		UnstoppableJobs uint32 `haproxy:"Unstoppable Jobs"`

		// Listeners: Current number of active listeners on the current worker process
		Listeners uint32 `haproxy:"Listeners"`

		// ActivePeers: Current number of verified active peers connections on the current worker process
		ActivePeers uint32 `haproxy:"ActivePeers"`

		// ConnectedPeers: Current number of peers having passed the connection step on the current worker process
		ConnectedPeers uint32 `haproxy:"ConnectedPeers"`

		// DroppedLogs: Total number of dropped logs for current worker process since started
		DroppedLogs uint32 `haproxy:"DroppedLogs"`

		// BusyPolling: 1 if busy-polling is currently in use on the worker process, otherwise zero (config.busy-polling)
		BusyPolling uint32 `haproxy:"BusyPolling"`

		// FailedResolutions: Total number of failed DNS resolutions in current worker process since started
		FailedResolutions uint32 `haproxy:"FailedResolutions"`

		// TotalBytesOut: Total number of bytes emitted by current worker process since started
		TotalBytesOut uint64 `haproxy:"TotalBytesOut"`

		// TotalSplicedBytesOut: Total number of bytes emitted by current worker process through a kernel pipe since started
		TotalSplicedBytesOut uint64 `haproxy:"TotalSplicedBytesOut"`

		// BytesOutRate: Number of bytes emitted by current worker process over the last second
		...
		// CumRecvLogs: Total number of log messages received by log-forwarding listeners on this worker process since started
		...
		// Niced_tasks: Total number of active tasks+tasklets in the current worker process (Run_queue) that are niced
		NicedTasks uint32 `haproxy:"Niced_tasks"`

		// CurrStreams: Current number of streams on this worker process
		CurrStreams uint64 `haproxy:"CurrStreams"`

		// CumStreams: Total number of streams created on this worker process since started
...
func (conn *Conn) ShowInfo() (Info, error) {
	var out Info

	rawOut, err := conn.doCommand(cmdShowInfoTyped)
	if err != nil {
		return out, err
	}
	...
	for _, rawLine := range strings.Split(rawOut, "\n") {
		...
		case reflect.TypeOf(time.Duration(0)):
			...
			d, err := parseHAProxyUptime(fieldValue)
			if err != nil {
...
func parseHAProxyUptime(s string) (time.Duration, error) {
	// Format: "0d 0h58m52s" (days + hours + minutes + seconds)
	// We'll parse very strictly.
	var d, h, m, sec int64
	_, err := fmt.Sscanf(strings.TrimSpace(s), "%dd %dh%dm%ds", &d, &h, &m, &sec)
	if err != nil {
		// some builds show "0d 0h46m36s" with a space after 'd'
		_, err2 := fmt.Sscanf(strings.TrimSpace(s), "%dd %dh%dm%ds", &d, &h, &m, &sec)
		if err2 != nil {
			return 0, err
		}
	}
