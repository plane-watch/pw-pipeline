...
func TestNewBeastMsgModeAC(t *testing.T) {
	_, err := Decode(beastModeAc)

	if !errors.Is(err, ErrUnhandledBeastFrame) {
		t.Error("did not error when getting an unhandled beast msg")
		return
	}
}

func TestNewBeastMsgModeSShort(t *testing.T) {
	f, err := Decode(beastModeSShort)
	...
	if f.DF != 11 {
		t.Error("Expected beastModeSShort to be a DF11")
	}

	df, err := f.DecodeDF11()
	if err != nil {
		t.Errorf("Failed to decode DF0: %s", err)
	}
	if df.ICAO != 0x7C49F8 {
		t.Errorf("ICAO failed to decode properly. got %X, expected: 0x7C49F8", df.ICAO)
	}
...
func TestNewBeastMsgModeSLong(t *testing.T) {
	...
	if f.DF != 17 {
		t.Errorf("Expected beastModeSLong to be DF17, got %d", f.DF)
	}
...
func BenchmarkNewFrameAndDecode(b *testing.B) {
	for _, name := range keys {
		arg := messages[name]
		b.Run(name, func(bb *testing.B) {
			var frame modes2.AvrFrame
			var err error
			for n := 0; n < bb.N; n++ {
				frame, err = Decode(arg)
				if nil != err {
					b.Error(err)
				}
				...
				switch frame.DF {
				case 0:
					_, err = frame.DecodeDF0()
				case 4:
					_, err = frame.DecodeDF4()
				case 5:
					_, err = frame.DecodeDF5()
				case 11:
					_, err = frame.DecodeDF11()
				case 16:
					_, err = frame.DecodeDF16()
				case 17:
					_, err = frame.DecodeADSB()
				case 18:
					_, err = frame.DecodeDF18()
				case 20:
					_, err = frame.DecodeDF20()
				case 21:
					_, err = frame.DecodeDF21()
				case 24:
					_, err = frame.DecodeDF24()
				}
				if err != nil {
					bb.Error(err)
				}
